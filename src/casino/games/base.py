...
Game-type catalog and abstract ``BaseGame`` for the casino.

The catalog is intentionally minimal: an enum of game types, an
enum of game actions, and a ``GAME_ACTIONS`` mapping declaring which
actions each game supports. The ``BaseGame`` ABC stays here because
it is a casino-specific primitive; concrete games live in their own
packages (``casino.blackjack``, ``casino.poker``, etc.).
"""
...
GAME_ACTIONS: dict[GameType, list[GameAction]] = {
    ...
    GameType.TICTACTOE: [GameAction.MOVE, GameAction.JOINT, GameAction.RESIGN],
}
